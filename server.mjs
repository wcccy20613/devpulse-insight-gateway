...
const MAX_MODEL_README_CHARS = 28_000;
const README_LEADING_CHARS = 8_000;
const MAX_PRIORITY_SECTION_CHARS = 3_500;
const PRIORITY_HEADING = /(install|getting started|quick start|usage|configuration|config|environment|api|cli|sdk|example|architecture|design|deploy|integration|requirement|compatibility|limitation|security|troubleshoot|license|安装|快速开始|入门|用法|使用|配置|环境|接口|示例|架构|设计|部署|集成|依赖|要求|兼容|限制|安全|故障|许可)/i;
...
export function compactReadme(readme, maxChars = MAX_MODEL_README_CHARS) {
  const source = typeof readme === "string" ? readme.trim() : "";
  if (!source || source.length <= maxChars) return source;

  const leadingLength = Math.min(README_LEADING_CHARS, maxChars);
  let compacted = source.slice(0, leadingLength).trimEnd();
  const headings = [...source.matchAll(/^#{1,6}\s+(.+)$/gm)];

  for (let index = 0; index < headings.length && compacted.length < maxChars; index += 1) {
    const heading = headings[index];
    if (heading.index < leadingLength || !PRIORITY_HEADING.test(heading[1])) continue;

    const sectionEnd = headings[index + 1]?.index ?? source.length;
    const section = source
      .slice(heading.index, sectionEnd)
      .trim()
      .slice(0, MAX_PRIORITY_SECTION_CHARS);
    if (!section) continue;

    const remaining = maxChars - compacted.length - 2;
    if (remaining <= 0) break;
    compacted += `\n\n${section.slice(0, remaining)}`;
  }

  const tailLength = Math.min(2_500, maxChars - compacted.length - 2);
  if (tailLength > 200 && compacted.length < source.length - tailLength) {
...
async function generateInsight(input) {
  ...
  const modelReadme = compactReadme(input.readme);
